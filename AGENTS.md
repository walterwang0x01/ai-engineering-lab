...
全站待审题在 400 道以上，一次过完不现实。加 `--path` 只看学习路线上的那 27 篇
（约 60 道待审）：过审的收益高度集中在路线上——那是新手唯一被引导着走完的一段，
其余笔记是按需查阅的。把清单收到能坐下来做完的规模，门禁才真的会被执行；
一个没人跑得完的门禁和一个不存在的门禁没有区别。
...
### 21. 没有吉祥物，「不要卡通游戏风」没有例外

曾经有一个 `src/lib/components/Mascot.svelte`（神经元：ReLU 折线当身体，
带眼睛、嘴和光晕），作为「不要卡通游戏风」的唯一例外记录在这里。**已删除。**

它当初的正当性是「身体就是 ReLU 折线，所以是内容而不是装饰」。这个理由只在
`BackpropExplorer` 里 44px、且紧邻讲死亡 ReLU 的正文时才成立。但同一个组件还被
放进了全站导航栏的 32px 位置——那个尺寸下没人能把它读成 ReLU 折线，它只剩
「一条紫色折线加一张笑脸」。**一条只在单一上下文成立的例外，被用在了它不成立的
地方，就不再是例外而是漏洞。**

现在的规则很简单：**站内不做拟人化图形，不画五官。**

- `BackpropExplorer` 的存活 / 死亡由左侧色条（绿 / 红）加正文承担，信息没丢——
  原本吉祥物也只是「同一信息的视觉编码，不是额外信息」。
- 品牌标记只保留纯几何形状：`static/favicon.svg` 与 `scripts/generate-og.mjs`
  里的 `brandMark()`，都是「ReLU 折线 + 抬起的输出端」，没有五官。
  这两处是各自独立的手抄副本，改形状时要一起改。
- 导航栏只有字标。窄屏（≤480px）**不要**把字标视觉隐藏——那条规则原本依赖
  「隐藏字标后还剩吉祥物」，吉祥物没了就只剩一个看不见却能点的 44px 空白方块。
  改成缩小字号。
...
低饱和、专业感的底子保留（参考 Linear / Vercel）。不做拟人化图形，见第 21 条。
